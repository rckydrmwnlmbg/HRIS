...
export default function KaryawanEditPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/karyawan/${id}`).then(r => r.json()),
      fetch('/api/master').then(r => r.json())
    ]).then(([empData, masterData]) => {
      if (empData.error) {
    ...
    return (
       <div className="animate-fadeIn">
         ...
         <div className="glass-card" style={{ padding: '60px', textAlign: 'center' }}>
            ...
            <div>{lang === 'id' ? 'Memuat data...' : 'Loading data...'}</div>
         </div>
       </div>
    );
  }

  if (error || !form.EMP_CD) {
    return (
      <div className="animate-fadeIn">
        <div className="page-header">
          <button className="btn btn-secondary" onClick={() => router.back()}>
        ...
        <div className="glass-card">
          <div className="empty-state">{lang === 'id' ? 'Karyawan tidak ditemukan' : 'Employee not found'}: {id}</div>
        </div>
      </div>
    );
  }
    ...
    <div className="animate-fadeIn">
      <div className="page-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            <h1 className="page-title">{lang === 'id' ? 'Edit Karyawan' : 'Edit Employee'}</h1>
            <p className="page-subtitle">{form.EMP_CD} - {form.EMP_NM}</p>
          </div>
        </div>
          ...
          {activeTab === 'pekerjaan' && (
            <div className={styles.formGrid}>
              ...
              <div className="form-group">
                ...
                <input type="date" className="form-input" value={formatDateForInput(form.DT_ENTRY)} onChange={e => set('DT_ENTRY', e.target.value)} />
              </div>
              <div className="form-group">
                <label className="form-label">{lang === 'id' ? 'Tanggal Resign' : 'Resignation Date'}</label>
                <input type="date" className="form-input" value={formatDateForInput(form.DT_RSG)} onChange={e => set('DT_RSG', e.target.value)} disabled={!!form.Act_NonAct} />
                {!!form.Act_NonAct && <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{lang === 'id' ? 'Hanya bisa diisi jika karyawan tidak aktif' : 'Only fillable if inactive'}</div>}
              </div>
          ...
          {activeTab === 'pribadi' && (
            <div className={styles.formGrid}>
              ...
              <div className="form-group">
                ...
                <input type="date" className="form-input" value={formatDateForInput(form.DT_BRT)} onChange={e => set('DT_BRT', e.target.value)} />
              </div>
