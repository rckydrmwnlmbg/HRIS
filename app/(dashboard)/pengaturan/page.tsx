    ...
    <div className="animate-fadeIn">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        ...
        <div className="glass-card" style={{ padding: '16px 18px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', background: 'var(--bg-subtle)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }}>
            <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,var(--accent-light),var(--accent))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', fontWeight: 700, color: '#fff' }}>
              {user?.nama?.charAt(0) || 'U'}
            </div>
            <div>
              <div style={{ fontWeight: 650, fontSize: '12.5px' }}>{user?.nama || '—'}</div>
              <div style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>{user?.role || '—'} · {user?.username || '—'}</div>
              <span className="badge badge-info" style={{ marginTop: '3px', fontSize: '9.5px' }}>{lang === 'id' ? 'Akses' : 'Access'}: {user?.Gr_Id || '—'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card" style={{ padding: '16px 18px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {[
              ...
              <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 10px', background: 'var(--bg-subtle)', borderRadius: 'var(--radius-sm)', fontSize: '11.5px' }}>
