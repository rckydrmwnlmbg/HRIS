...
async function check() {
  ...
  console.log('=== 1. SEARCH EMP_TABLE FOR WIDYA / ETIKA ===');
  ...
  console.log('=== 2. ALL TABLES IN PAYROLLSYS ===');
  ...
  if (emps.recordset.length > 0) {
    ...
    console.log(`=== 3. TR_ABSEN SUMMARY FOR ${nik} IN JUNE 2026 ===`);
    const sum = await pool.request().query(`
      SELECT 
        COUNT(CASE WHEN WORK_IN IS NOT NULL THEN 1 END) AS TOTAL_HADIR,
    ...
    `);
    console.log(JSON.stringify(sum.recordset[0], null, 2));

    console.log(`=== 4. TR_ABSEN ALL ROWS FOR ${nik} IN JUNE 2026 ===`);
